...
class DataProblem():
  ...
  def __init__(self, depot, loc, prize, max_length, depot2=None):
    """Initializes the data for the problem"""
    # Locations in block unit
    if depot2 is None:
        self._locations = [(float_to_scaled_int(l[0]), float_to_scaled_int(l[1])) for l in [depot] + loc]
    else:
        self._locations = [(float_to_scaled_int(l[0]), float_to_scaled_int(l[1])) for l in [depot] + loc + [depot2]]
...
def solve_op_ortools(depot, loc, prize, max_length, sec_local_search=0, depot2=None):
    data = DataProblem(depot, loc, prize, max_length, depot2=depot2)

    # Create Routing Model
    if depot2 is None:
        manager = pywrapcp.RoutingIndexManager(data.num_locations, data.num_vehicles, data.depot)
    else:
        manager = pywrapcp.RoutingIndexManager(data.num_locations, data.num_vehicles, [data.depot], [data.depot2])
