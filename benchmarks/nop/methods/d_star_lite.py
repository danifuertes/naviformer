...
class DStarLite:
    ...
    def __init__(self, ox: list, oy: list, start: list, goal: list):
    ...
    def compare_keys(self, key_pair1, key_pair2):
    ...
    def planning(self, sx, sy, gx, gy, spoofed_ox=None, spoofed_oy=None):
        start, goal = Node(x=round(sx), y=round(sy)), Node(x=round(gx), y=round(gy))
        if spoofed_oy is None:
            spoofed_oy = []
        if spoofed_ox is None:
            spoofed_ox = []
...
def main():
    ...
    dstarlite.planning(sx, sy, gx, gy, spoofed_ox=spoofed_ox, spoofed_oy=spoofed_oy)
