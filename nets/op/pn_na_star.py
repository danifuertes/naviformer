...
class PNNAStar(nn.Module):
    ...
    def forward(self, batch: dict | torch.Tensor, env: Any, test: bool = True) -> \
        ...
        while not done:
            ...
            action, path, log_prob, hidden_dec = self.step(state, embeddings, hidden_dec)
    ...
    def step(self, state: Any, embeddings: torch.Tensor, hidden_dec: Tuple) -> Tuple[torch.Tensor, torch.Tensor, Tuple]:
        ...
        # Transformer decoder
        log_probs_node, selected_node, _, selected_direction, hidden_dec = self.decoder(state, embeddings, hidden_dec)
        ...
        return selected_node, selected_direction, log_prob, hidden_dec
