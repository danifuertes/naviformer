...
class Encoder(nn.Module):
    ...
    def forward(self, x, hidden):
        ...
        output, hidden2 = self.lstm(x, hidden)
        hidden = hidden2
...
class PNTransPath(nn.Module):
    ...
    def __init__(self,
        ...
        """
        super(PNTransPath, self).__init__()
        ...
        self.transpath = load_transpath_models(device=device)
    ...
    def forward(self, batch: dict | torch.Tensor, env: Any, test: bool = True) -> \
        ...
        while not done:
            ...
            action, path, log_prob, hidden_dec, state = self.step(state, embeddings, hidden_dec)
    ...
    def step(self, state: Any, embeddings: torch.Tensor, hidden_dec: Tuple) -> Tuple[torch.Tensor, torch.Tensor, Tuple]:
        ...
        # Transformer decoder
        log_probs_node, selected_node, _, selected_direction, hidden_dec, state = self.decoder(state, embeddings, hidden_dec)
        ...
        return selected_node, selected_direction, log_prob, hidden_dec, state
    ...
    def decoder(self, state: Any, embeddings: torch.Tensor, hidden_dec: Tuple) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, Tuple]:
        ...
        # Return actions and log probabilities
        return log_probs_node, selected_node, log_probs_direction, selected_direction, hidden_dec, state
    ...
    def predict_direction(self, state: Any, next_node: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            path = torch.stack((start, state.depot_ini), dim=1)
            selected_direction = torch.cat((
                torch.zeros_like(path[..., 0, None]), path
            ), dim=-1)
            state = state._replace(length=100)
        return selected_direction, state
