...
def global_vars() -> dict:
    ...
    return {
        ...
        'MODELS': ['naviformer', 'pn', 'gpn', 'naviformer2step', 'pn2step', 'gpn2step',
                   'naviformer_a_star', 'naviformer_na_star', 'naviformer_transpath',
                   'pn_na_star', 'pn_a_star', 'pn_transpath', 'gpn_na_star', 'gpn_a_star', 'gpn_transpath'],
        'NORM': ['batch', 'instance'],
...
def check_make_data_options(opts: argparse.Namespace) -> None:
    ...
    assert len(opts.num_obs) == 2,                          f"num_obs must contain 2 values, found: {len(opts.num_obs)}"
    assert 0 <= opts.num_obs[0] <= opts.num_obs[1],         f"num_obs must be a non-negative range, found: {opts.num_obs}"
    assert len(opts.rad_obs) == 2,                          f"rad_obs must contain 2 values, found: {len(opts.rad_obs)}"
    assert 0 <= opts.rad_obs[0] <= opts.rad_obs[1] <= .4,   f"rad_obs must be a non-negative range smaller than 0.4, found: {opts.rad_obs}"
